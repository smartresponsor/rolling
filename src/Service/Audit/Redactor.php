...
final class Redactor
{
    ...
    private function maskPath(array $event, string $path): array
    {
        ...
        for ($i = 0; $i < count($parts) - 1; $i++) {
            $k = $parts[$i];
            if (!is_array($ref) || !array_key_exists($k, $ref)) return $event;
            $ref =& $ref[$k];
        }
    ...
    {
        if (is_string($v)) return str_repeat('*', min(8, max(3, (int)(strlen($v) / 2))));
        if (is_array($v)) return '[MASKED_ARRAY]';
        if (is_object($v)) return '[MASKED_OBJECT]';
        return '[MASKED]';
    }
}
