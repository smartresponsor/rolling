...
final class ConfigLoader
{
    ...
    public function loadJsonFile(string $path): array
    {
        ...
        if ($raw === false) throw new RuntimeException("perm_config_read_failed:$path");
        $cfg = json_decode($raw, true);
        if (!is_array($cfg) || !isset($cfg['permissions']) || !is_array($cfg['permissions'])) throw new RuntimeException("perm_config_bad_json");
        $out = [];
