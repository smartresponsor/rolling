...
final class SimpleTenantContextResolver implements TenantContextResolverInterface
{
    ...
    {
        if (isset($context['tenant']) && is_string($context['tenant'])) return $context['tenant'];
        if (isset($resource['tenant']) && is_string($resource['tenant'])) return $resource['tenant'];
        if (isset($subject['tenant']) && is_string($subject['tenant'])) return $subject['tenant'];
        return null;
    }
}
